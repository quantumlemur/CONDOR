...
class Rotor:
    ...
    def trim(self, tolerance, V, speedOfSound, rho, Fx, Fy, Fz):
        ...
        while (abs(F_x-Fx)>1 or abs(F_y-Fy)>1 or abs(F_z-Fz)>1) and loops<1000:
            ...
            alphaEffective = 1/U_T * (omega*r*(theta_0 + blade.theta_tw*r/R + (theta_1c-beta_1s)*cospsi + (theta_1s+beta_1c)*sinpsi)*V*(theta_0 + blade.theta_tw*r/R)*sinpsi + V*(theta_1c-beta_1s)*cospsi*sinpsi + V*(theta_1s+beta_1c)*sinpsi**2 - V*beta_0*cospsi - V*alpha_TPP - inflow)
            ...
            mach = U_total / speedOfSound # Assumes U_T>>U_P, which is only untrue if U_T is small (i.e. on the retreating side in high-speed flight), in which case mach numbers will be small anyway, so should be ok.
